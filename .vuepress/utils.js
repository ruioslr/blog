...
const getItems = (dir) => {
  ...
  const files = fs
    .readdirSync(dir)
    .filter((_) => fs.statSync(path.resolve(rootPath, dir) + "/" + _).isFile())
    .map((_) => _.slice(0, -3));
  const dirs = fs
    .readdirSync(dir)
    .filter((_) =>
      fs.statSync(path.resolve(rootPath, dir) + "/" + _).isDirectory()
    );
...
const getSidebar = () => {
  ...
  const allFiles = fs
    ...
    .filter((_) => !excludeDir.includes(_));
  const dirs = allFiles.filter((_) =>
    fs.statSync(rootPath + "/" + _).isDirectory()
  );
  dirs.forEach((dir) => {
    res.push({
      title: dir,
...
const getNav = () => {
  ...
  // })
  // return res;
  return [{ text: "Github", link: "https://github.com/ruioslr/blog" }];
};
